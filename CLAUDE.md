...
| `ServiceResponseExtensions` | `Nostreets.Extensions.Core.Models.Responses` (file `ServiceResponseExtensions.cs`) | **The safe-consume vocabulary for `ServiceResponse` — reach for these instead of hand-rolling null/success checks.** `HasError()` / `IsUsable()` / `GetDataOr(fallback)` / `GetDataOrDefault()` / `TryGetData(out data)` / `ErrorSummary()`. **The `<T>` overloads also treat a null `Data` as unusable**, so `resp.IsUsable()` replaces the whole `!IsSuccessful \|\| Data == null` idiom — and additionally catches `Errors.Count > 0`, which that hand-rolled form misses. All receivers are nullable and null-safe, so no `?.` is needed. ⚠ **They resolve by IMPORTED NAMESPACE, not by receiver type** — a file that only ever holds responses in a `var` may have no `using Nostreets.Extensions.Core.Models.Responses;` and will fail with CS1061; add the using rather than re-rolling the check. |
